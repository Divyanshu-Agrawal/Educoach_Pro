...
public class StudentAttendanceActivity extends AppCompatActivity {
	...
	String userrType;
	String userPassword;
	String userSchoolId, stdSubject;
...
//		showAttendance(attendance);
		...
		if (isInternetOn()) {
			...
			if (userrType.equals("Teacher") || userrType.equals("Admin")) {
				...
				submit_lieave_tv.setVisibility(View.GONE);
				view_leaveList.setVisibility(View.GONE);
				studentId = getIntent().getStringExtra("studentId");
				studentImg = getIntent().getStringExtra("studentImg");
				stdSubject = getIntent().getStringExtra("subject");
				tool_title.setText(stdSubject);
				GetAllAttandance b1 = new GetAllAttandance(StudentAttendanceActivity.this);
				b1.execute(studentId, stdSubject);
			} else {
				iv_edit.setVisibility(View.VISIBLE);
				submit_lieave_tv.setVisibility(View.VISIBLE);
				view_leaveList.setVisibility(View.VISIBLE);
				stdSubject = getIntent().getStringExtra("subject");
				tool_title.setText(stdSubject);
				GetAllAttandance b1 = new GetAllAttandance(StudentAttendanceActivity.this);
				b1.execute(userId, stdSubject);
			}
		} else {
			cal_month = (GregorianCalendar) GregorianCalendar.getInstance();
			cal_month_copy = (GregorianCalendar) cal_month.clone();
			mSwipeRefreshLayout.setRefreshing(false);
			showAttendance(attendance);
			Toast.makeText(StudentAttendanceActivity.this, "No network Please connect with network for update", Toast.LENGTH_SHORT).show();
		}
		...
		if (userrType.equals("Student")) {
			...
			if (studentImg.equals("0")) {
				iv_user_img.setImageDrawable(getResources().getDrawable(R.drawable.user_place_hoder));
			} else if (!TextUtils.isEmpty(studentImg)) {
				String url = settings.getString("imageUrl", "") + settings.getString("userSchoolId", "") + "/users/students/profile/" + studentImg;
				Picasso.with(this).load(url).error(R.drawable.dummy).placeholder(R.drawable.dummy).into(iv_user_img);
			} else {
				iv_user_img.setImageDrawable(getResources().getDrawable(R.drawable.user_place_hoder));
			}
		...
		iv_edit.setOnClickListener(view -> {
			if (isInternetOn()) {
				if (ContextCompat.checkSelfPermission(view.getContext(), Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
					ActivityCompat.requestPermissions(StudentAttendanceActivity.this, new String[]{Manifest.permission.CAMERA, Manifest.permission.WRITE_EXTERNAL_STORAGE}, 0);
				}
	...
	private void showAttendance(String attendance) {
		if (attendance.equals("{\"result\":null}")) {
			...
			percent2.setText("0%");
			eventDetails();
			Toast.makeText(StudentAttendanceActivity.this, "No Attendance", Toast.LENGTH_SHORT).show();
		} else {
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == CropImage.CROP_IMAGE_ACTIVITY_REQUEST_CODE) {
			...
			if (resultCode == RESULT_OK) {
				...
				try {
					File actualImage = FileUtil.from(StudentAttendanceActivity.this, filePath);
					File compressedImage = new Compressor(StudentAttendanceActivity.this)
							.setMaxWidth(640)
							.setMaxHeight(480)
							.setQuality(75)
					...
					bitmap = MediaStore.Images.Media.getBitmap(StudentAttendanceActivity.this.getContentResolver(), Uri.fromFile(compressedImage));
					
					UpdateProfileImage updateProfileImage = new UpdateProfileImage(StudentAttendanceActivity.this, compressedImage);
				...
				Toast.makeText(StudentAttendanceActivity.this, "" + error, Toast.LENGTH_SHORT).show();
			}
		}
	}
		...
		protected void onPostExecute(String result) {
			...
			try {
				...
				if (jsonObject.getString("success").equals("true")) {
					...
					editor.commit();
					Toast.makeText(StudentAttendanceActivity.this, "Your Image is Updated", Toast.LENGTH_SHORT).show();
				} else {
					Toast.makeText(StudentAttendanceActivity.this, "Not uploaded image is too large", Toast.LENGTH_SHORT).show();
				}
			} catch (JSONException e) {
				e.printStackTrace();
			}
