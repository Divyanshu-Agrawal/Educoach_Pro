...
import static com.aaptrix.tools.HttpUrl.SUBMIT_LEAVE;
...
public class LeaveApplicationActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.leave_application_main_layout);
		...
		leaveStartDate.setFocusable(false);
		...
		leaveStartDate.setOnClickListener(view -> {
			...
			DatePickerDialog mDatePicker = new DatePickerDialog(
					LeaveApplicationActivity.this, R.style.AlertDialogCustom1, (datepicker, selectedyear, selectedmonth, selectedday) -> {
		...
		btn_send.setOnClickListener(view -> {
			...
			if (TextUtils.isEmpty(str_subject)) {
				Toast.makeText(LeaveApplicationActivity.this, "Please enter subject", Toast.LENGTH_SHORT).show();
			} else if (TextUtils.isEmpty(str_compose)) {
				Toast.makeText(LeaveApplicationActivity.this, "Please enter leave details", Toast.LENGTH_SHORT).show();
			} else if (TextUtils.isEmpty(str_leaveStartDate)) {
				Toast.makeText(LeaveApplicationActivity.this, "Please select leave start date", Toast.LENGTH_SHORT).show();
			} else if (TextUtils.isEmpty(str_leaveEndDate)) {
				Toast.makeText(LeaveApplicationActivity.this, "Please select leave end date", Toast.LENGTH_SHORT).show();
			} else {
				layout.setVisibility(View.VISIBLE);
				layout.bringToFront();
				boolean bool = CheckDates(str_leaveStartDate, str_leaveEndDate);
				if (bool) {
					LayoutInflater factory = LayoutInflater.from(LeaveApplicationActivity.this);
					...
					alert = new AlertDialog.Builder(LeaveApplicationActivity.this, R.style.DialogTheme);
					...
					Toast.makeText(LeaveApplicationActivity.this, "End date should be more than start date", Toast.LENGTH_SHORT).show();
				}
			}
		...
		public void onClick(View v) {
			...
			if (!TextUtils.isEmpty(mValue)) {
				if (old.equals(userPassword)) {
					alertDialog.dismiss();
					try {
						SendLeaveApplication b1 = new SendLeaveApplication(LeaveApplicationActivity.this);
					...
					Toast.makeText(LeaveApplicationActivity.this, "Password Wrong", Toast.LENGTH_SHORT).show();
				}
			} else {
				Toast.makeText(LeaveApplicationActivity.this, "Please enter Password", Toast.LENGTH_SHORT).show();
			}
		}
	}
	...
	public void setupUI(View view) {
		if (!(view instanceof EditText)) {
			view.setOnTouchListener((v, event) -> {
				hideSoftKeyboard(LeaveApplicationActivity.this);
				return false;
			});
		}
	...
	public class SendLeaveApplication extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... params) {
			...
			String userTeacherName = params[6];
			String notiImage = "0";
			...
			try {
				...
				for (int i = 0; i < imageArray.size(); i++) {
					try {
						...
						HttpPost httppost = new HttpPost(SUBMIT_LEAVE);
				...
				try {
					...
					entityBuilder.addTextBody("userTeacherName", userTeacherName);
					HttpEntity entity = entityBuilder.build();
					httppost.setEntity(entity);
					HttpResponse response = httpclient.execute(httppost);
					HttpEntity httpEntity = response.getEntity();
			...
			if (result.contains("\"submitted\"")) {
				...
				new CountDownTimer(4000, 1000) {
					...
					public void onFinish() {
						mSwipeRefreshLayout.setRefreshing(false);
						Intent i = new Intent(LeaveApplicationActivity.this, StudentAttendanceActivity.class);
						i.putExtra("subject", " ");
						startActivity(i);
